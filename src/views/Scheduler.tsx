  ...
  const handleAIAutocomplete = async () => {
    ...
    try {
      ...
      if (data.copy) {
        ...
        const engineLabel = data.engine === 'groq' 
          ? `Groq - ${data.model || 'LLaMA 3.3'}` 
          : 'Gemini 3.5 Flash';
        
        addToast?.(`${t('Copywriting successfully generated by AI!')} (${engineLabel})`, 'success');
        
        if (data.engine === 'gemini' && data.groqError) {
          console.warn('Groq fell back to Gemini. Groq diagnostic log:', data.groqError);
        }
      } else {
